...
import CrosspostSDK, { type PlatformConfig, type PostContent } from '../src/standalone';
import { validatePostContent, sanitizePostContent } from '../src/utils/validation';
...
const platformConfigs: PlatformConfig[] = [
	{
		...
		credentials: {
			...
			accessTokenSecret: process.env.TWITTER_ACCESS_TOKEN_SECRET || 'your-twitter-access-token-secret',
		},
	},
	{
		platform: 'linkedin',
		enabled: true,
		credentials: {
			accessToken: process.env.LINKEDIN_ACCESS_TOKEN || 'your-linkedin-access-token',
		},
	},
	{
		platform: 'facebook',
		enabled: true,
		credentials: {
			accessToken: process.env.FACEBOOK_ACCESS_TOKEN || 'your-facebook-access-token',
	...
	},
	{
		...
		enabled: true,
		credentials: {
			accessToken: process.env.INSTAGRAM_ACCESS_TOKEN || 'your-instagram-access-token',
			instagramAccountId: process.env.INSTAGRAM_ACCOUNT_ID || 'your-instagram-account-id',
		},
	},
	{
		platform: 'mastodon',
		enabled: true,
		credentials: {
			accessToken: process.env.MASTODON_ACCESS_TOKEN || 'your-mastodon-access-token',
...
export async function demoScheduledPosting() {
	...
	try {
		...
		console.log(`📅 Scheduling post for: ${new Date(scheduledAt).toISOString()}`);
...
export async function demoAnalytics() {
	...
	try {
		...
		for (const post of successfulPosts) {
			...
			const analytics = await crosspost.getAnalytics(post.postId, post.platform);
			...
			if (analytics) {
				...
				console.log(`  📅 Last updated: ${new Date(analytics.lastUpdated).toISOString()}`);
			} else {
	...
	try {
		...
		Object.entries(stats.platformBreakdown).forEach(([platform, breakdown]) => {
			...
			if (breakdown.posts > 0) {
				const successRate = ((breakdown.successes / breakdown.posts) * 100).toFixed(1);
...
export async function demoCredentialValidation() {
	...
	try {
		...
		if (validPlatforms === 0) {
			console.log('⚠️ No valid credentials found. Please check your configuration.');
		} else if (validPlatforms < totalPlatforms) {
			console.log('⚠️ Some platforms have invalid credentials. Check your configuration.');
		} else {
...
export async function runComprehensiveDemo() {
	...
	try {
		...
		console.log('=============================================');
		console.log('All features have been demonstrated. The crosspost plugin is ready for production use!');
