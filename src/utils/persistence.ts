...
export class MemoryPersistenceAdapter implements PersistenceAdapter {
	...
	async getAnalytics(postId: string, platform: string): Promise<Analytics | null> {
...
export class FilePersistenceAdapter implements PersistenceAdapter {
	...
	async saveScheduledPost(post: ScheduledPost): Promise<void> {
		const posts = await this.readJsonFile<Record<string, ScheduledPost>>(this.scheduledPostsFile, {});
		posts[post.id] = post;
	...
	async saveAnalytics(analytics: Analytics): Promise<void> {
		const allAnalytics = await this.readJsonFile<Record<string, Analytics>>(this.analyticsFile, {});
		const key = `${analytics.platform}:${analytics.postId}`;
	...
	async savePostResult(result: PostResult): Promise<void> {
		const results = await this.readJsonFile<PostResult[]>(this.postResultsFile, []);
		results.push(result);
	...
	async incrementStat(key: string, value: number = 1): Promise<void> {
		const stats = await this.readJsonFile<Record<string, number>>(this.statsFile, {});
...
export class PersistentScheduler {
	...
	constructor(persistence: PersistenceAdapter = new MemoryPersistenceAdapter()) {
	...
	async executeScheduledPost(id: string, executeCallback: (post: ScheduledPost) => Promise<PostResult[]>): Promise<ScheduledPost | null> {
		const post = await this.persistence.getScheduledPost(id);
		if (!post || post.status !== 'pending') {
			return null;
		}
