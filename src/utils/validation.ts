...
export function validatePostContent(content: PostContent): void {
	...
	if (!content.text || typeof content.text !== 'string') {
		throw new ValidationError('Text content is required and must be a string', 'text');
	}
	...
	if (content.text.length > 10000) {
		throw new ValidationError('Text content exceeds maximum length of 10,000 characters', 'text');
	}
	...
	if (content.hashtags) {
		...
		for (const hashtag of content.hashtags) {
			...
			if (hashtag.length > 100) {
				throw new ValidationError('Hashtag too long (max 100 characters)', 'hashtags');
			}

			// Check for invalid characters
			if (!/^[a-zA-Z0-9_]+$/.test(hashtag.replace(/^#/, ''))) {
				throw new ValidationError(`Invalid hashtag format: ${hashtag}`, 'hashtags');
			}
		}
	}
	...
	if (content.mentions) {
		...
		for (const mention of content.mentions) {
			...
			if (mention.length > 50) {
				throw new ValidationError('Mention too long (max 50 characters)', 'mentions');
			}
		}
	}
	...
	if (content.images) {
		...
		for (const image of content.images) {
			if (typeof image !== 'string') {
				throw new ValidationError('All images must be strings (URLs or base64)', 'images');
			}

			// Check if it's a URL or base64
			if (!image.startsWith('data:image/') && !isValidUrl(image)) {
				throw new ValidationError('Images must be valid URLs or base64 data URLs', 'images');
			}
		}
	}
}
...
export function validatePlatforms(platforms: Platform[]): void {
	...
	const validPlatforms: Platform[] = ['twitter', 'linkedin', 'facebook', 'instagram', 'mastodon', 'bluesky'];
	
	for (const platform of platforms) {
		if (!validPlatforms.includes(platform)) {
			throw new ValidationError(`Invalid platform: ${platform}. Valid platforms are: ${validPlatforms.join(', ')}`);
		}
	}
...
export function validateScheduledAt(scheduledAt: number): void {
	if (typeof scheduledAt !== 'number') {
		throw new ValidationError('Scheduled time must be a number (Unix timestamp)');
	}
	...
	if (scheduledAt > oneYearFromNow) {
		throw new ValidationError('Cannot schedule posts more than 1 year in advance');
	}
}
...
export function sanitizePostContent(content: PostContent): PostContent {
	return {
		...
		images: content.images ? content.images.filter(img => 
			typeof img === 'string' && (img.startsWith('data:image/') || isValidUrl(img))
		).slice(0, 4) : undefined,
	};
}
