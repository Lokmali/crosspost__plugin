...
export async function uploadMedia(
  ...
  switch (platform) {
    ...
    case "facebook":
      return uploadFacebookMedia(
        mediaData,
        accessToken,
        options.additionalParams?.pageId,
      );
    case "instagram":
      return uploadInstagramMedia(
        mediaData,
        accessToken,
        options.additionalParams?.instagramAccountId,
      );
    case "mastodon":
      return uploadMastodonMedia(
        mediaData,
        accessToken,
        options.additionalParams?.instanceUrl,
      );
    case "bluesky":
      return uploadBlueskyMedia(mediaData, accessToken);
    default:
      throw new MediaUploadError(
        `Media upload not supported for platform: ${platform}`,
        platform,
      );
  }
}
...
async function uploadTwitterMedia(
  ...
  try {
    ...
    if (typeof mediaData === "string") {
      ...
      formData.append(
        "media",
        new Blob([new Uint8Array(mediaData)], { type: "image/jpeg" }),
      );
    }
    ...
    throw new MediaUploadError(
      `Twitter media upload failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      "twitter",
    );
  }
}
...
async function uploadLinkedInMedia(
  ...
  try {
    ...
    throw new MediaUploadError(
      `LinkedIn media upload failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      "linkedin",
    );
  }
}

async function uploadFacebookMedia(
  mediaData: string | Buffer,
  accessToken: string,
  pageId?: string,
): Promise<MediaUploadResult> {
  if (!pageId) {
    throw new MediaUploadError(
      "Page ID is required for Facebook media upload",
      "facebook",
    );
  }
  ...
  try {
    ...
    throw new MediaUploadError(
      `Facebook media upload failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      "facebook",
    );
  }
}
...
async function uploadInstagramMedia(
  ...
  if (!instagramAccountId) {
    throw new MediaUploadError(
      "Instagram account ID is required for Instagram media upload",
      "instagram",
    );
  }
  ...
  try {
    ...
    throw new MediaUploadError(
      `Instagram media upload failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      "instagram",
    );
  }
}
...
async function uploadMastodonMedia(
  ...
  try {
    ...
    throw new MediaUploadError(
      `Mastodon media upload failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      "mastodon",
    );
  }
}

async function uploadBlueskyMedia(
  mediaData: string | Buffer,
  accessToken: string,
): Promise<MediaUploadResult> {
  try {
    ...
    throw new MediaUploadError(
      `Bluesky media upload failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      "bluesky",
    );
  }
}
...
export function validateMediaFile(
  ...
  if (size > maxSize) {
    throw new MediaUploadError(
      `File size (${size} bytes) exceeds maximum allowed size (${maxSize} bytes) for ${platform}`,
    );
  }

  if (!supportedTypes.includes(mimeType)) {
    throw new MediaUploadError(
      `File type ${mimeType} is not supported by ${platform}. Supported types: ${supportedTypes.join(", ")}`,
    );
  }
}
