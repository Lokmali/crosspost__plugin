...
export type CrosspostPluginClient = ContractRouterClient<
  typeof crosspostContract
>;
...
export default createPlugin({
    ...
    Effect.gen(function* () {
      ...
      const stats = {
        ...
        platformBreakdown: new Map<
          Platform,
          { posts: number; successes: number; failures: number }
        >(),
      };
                ...
                if (post.status === "pending" && post.scheduledAt <= now) {
                  try {
                    yield* Effect.tryPromise(() =>
                      client.executeScheduledPost(post.id),
                    );
                  } catch (error) {
                    console.error(
                      `Failed to execute scheduled post ${post.id}:`,
                      error,
                    );
                  }
                }
      ...
      console.log(
        `[CrosspostPlugin] Initialized with ${validPlatforms.length} valid platform(s)`,
      );
  ...
  createRouter: (context) => {
    ...
    // Get scheduled posts
    const getScheduledPosts = os.getScheduledPosts.handler(
      async ({ input }) => {
    ...
    const executeScheduledPost = os.executeScheduledPost.handler(
      async ({ input }) => {
    ...
    const cancelScheduledPost = os.cancelScheduledPost.handler(
      async ({ input }) => {
    ...
    const getPlatformStatus = os.getPlatformStatus.handler(async () => {
      const platforms = client.getSupportedPlatforms().map((platform) => ({
        ...
        characterLimit:
          platform === "twitter" ? 280 : platform === "linkedin" ? 3000 : 1000,
      }));
    ...
    const validateCredentials = os.validateCredentials.handler(
      async ({ input }) => {
        const platforms = input.platforms || client.getSupportedPlatforms();
    ...
    // Stream post events
    const streamPostEvents = os.streamPostEvents.handler(async function* ({
      input,
    }) {
    ...
    const bulkPost = os.bulkPost.handler(async ({ input }) => {
      ...
      for (const post of input.posts) {
        const postResults = await client.postToAll(
          post.content,
          post.platforms,
        );
    ...
    const healthCheck = os.healthCheck.handler(async () => {
      ...
      for (const platform of client.getSupportedPlatforms()) {
        const isHealthy =
          client.isPlatformConfigured(platform) && credentialResults[platform];
