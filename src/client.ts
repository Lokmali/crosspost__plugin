...
export class TwitterAdapter implements PlatformAdapter {
  ...
  async post(content: PostContent): Promise<PostResult> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Twitter API error: ${response.status} - ${errorData.detail || response.statusText}`,
        );
      }
  ...
  formatContent(content: PostContent): PostContent {
    ...
    if (content.hashtags && content.hashtags.length > 0) {
      const hashtags = content.hashtags.map((tag: string) =>
...
export class InstagramAdapter implements PlatformAdapter {
  ...
  async validateCredentials(): Promise<boolean> {
    ...
    try {
      const response = await fetch(
        `https://graph.facebook.com/v18.0/${this.instagramAccountId}?access_token=${this.accessToken}`,
      );
      return response.ok;
    } catch (error) {
  ...
  async post(content: PostContent): Promise<PostResult> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          `Instagram API error: ${response.status} - ${errorData.error?.message || response.statusText}`,
        );
      }
  ...
  async getPostAnalytics(postId: string): Promise<Analytics | null> {
    try {
      const response = await fetch(
        `https://graph.facebook.com/v18.0/${postId}/insights?metric=impressions,reach,engagement&access_token=${this.accessToken}`,
      );
      if (!response.ok) return null;
  ...
  async post(content: PostContent): Promise<PostResult> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          `Mastodon API error: ${response.status} - ${errorData.error || response.statusText}`,
        );
      }
  ...
  async post(content: PostContent): Promise<PostResult> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          `Bluesky API error: ${response.status} - ${errorData.message || response.statusText}`,
        );
      }
...
export class CrosspostClient {
  ...
  async executeScheduledPost(id: string): Promise<ScheduledPost | null> {
    ...
    try {
      ...
      scheduledPost.status = results.every((r) => r.success)
        ? "posted"
        : "failed";
